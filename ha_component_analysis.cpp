...
// Assuming structuralPath has a constructor that takes in
// a list of locations and a list of transitions.

#include <boost/make_shared.hpp> // make sure you include this
...
//        string model = "water_level_monitor_agent";
        string model = "warehouse_automation_agent";
...
//            infeasiblePaths.push_back(ptr);
        	...
        	for (size_t i = 2; i <= currentPathLocations.size(); ++i) {
				...
				if (infeasiblePaths.find(subPathKey) != infeasiblePaths.end()) {
					std::cout << "Subpath ending at location " << subPathLocations.back()->getName() << " is known to be infeasible." << std::endl;
...
//					std::cout << subXml_file << " is open" <<std::endl;
				...
				if (subPathFeasible) {
					...
					std::cout << "Subpath ending at location " << subPathLocations.back()->getName() << " is infeasible." << std::endl;
